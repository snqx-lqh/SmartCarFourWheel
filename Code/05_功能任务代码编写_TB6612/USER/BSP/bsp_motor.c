...
{
	if(*motor_pwm > MAX_PWM_SET*0.8)       *motor_pwm = MAX_PWM_SET*0.8;
	else if(*motor_pwm < -MAX_PWM_SET*0.8) *motor_pwm = -MAX_PWM_SET*0.8;
}
...
void motor_pwm_set(int16_t motorAPwm,int16_t motorBPwm,int16_t motorCPwm,int16_t motorDPwm)
{
	...
	if(motorAPwm>0)
	{
		MOTORA_IN1 = 1;
		MOTORA_IN2 = 0;
		MOTORA_PWM_SET(MOTORA_TIM,motorAPwm);
	}else 
	{
		MOTORA_IN1 = 0;
		MOTORA_IN2 = 1;
	...
	if(motorBPwm>0)
	{
		MOTORB_IN1 = 1;
		MOTORB_IN2 = 0;
		MOTORB_PWM_SET(MOTORB_TIM,motorBPwm);
	}else 
	{
		MOTORB_IN1 = 0;
		MOTORB_IN2 = 1;
	...
	if(motorCPwm>0)
	{
		MOTORC_IN1 = 0;
		MOTORC_IN2 = 1;
		MOTORC_PWM_SET(MOTORC_TIM,motorCPwm);
	}else 
	{
		MOTORC_IN1 = 1;
		MOTORC_IN2 = 0;
	...
	if(motorDPwm>0)
	{
		MOTORD_IN1 = 0;
		MOTORD_IN2 = 1;
		MOTORD_PWM_SET(MOTORD_TIM,motorDPwm);
	}else 
	{
		MOTORD_IN1 = 1;
		MOTORD_IN2 = 0;
...
void bsp_motor_dir_init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;	//端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;      //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;     //50M
	
	GPIO_Init(GPIOB, &GPIO_InitStructure);					      //根据设定参数初始化GPIOB 
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_11|GPIO_Pin_12;	//端口配置
